...
public class FoundationDBClientCache {
  ...
  private final LoadingCache<ClientKey, FoundationDBClient> clientCache =
      Caffeine.newBuilder().build(subDirectoryVisiblityPair -> {
        return new FoundationDBClient(
  ...
  protected FoundationDBClientCache() {}

  public FoundationDBClient getClient(
  ...
  public synchronized void close(
    ...
    final ClientKey key =
        new ClientKey(directory, visibilityEnabled, compactOnWrite, batchWriteSize);
    final FoundationDBClient client = clientCache.getIfPresent(key);
    if (client != null) {
